...
# --- Configuration ---
# psid 134 = Quadro RTX 5000 Series; pfid 1085 = RTX 5000.
# The DCH driver is unified across all current Quadro GPUs, so this pfid is
# a stable lookup key for "the latest Game Ready driver for current cards"
# rather than a per-GPU selector. Update only if NVIDIA retires PSID 134.
...
function Resolve-NvidiaQuadroLatest {
    ...
    try {
        ...
        Write-Log "Failed to resolve NVIDIA Quadro driver: $($_.Exception.Message)" -Level ERROR
        return $null
    }
}
...
function Invoke-StageNvidiaQuadro {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NVIDIA Graphics Driver (Quadro) - STAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""
    ...
    if (-not $release) { throw "Could not resolve latest NVIDIA Quadro driver." }
...
function Invoke-PackageNvidiaQuadro {
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NVIDIA Graphics Driver (Quadro) - PACKAGE phase"
    Write-Log ("=" * 60)
    Write-Log ""

    $release = Resolve-NvidiaQuadroLatest -Quiet
    if (-not $release) { throw "Could not resolve latest NVIDIA Quadro driver for manifest lookup." }
...
try {
    ...
    Write-Log ""
    Write-Log ("=" * 60)
    Write-Log "NVIDIA Quadro Auto-Packager starting"
    Write-Log ("=" * 60)
    Write-Log ""
